...
save_filename        = ['besselScan'];                                     % Pick a file to save the simulated movie as (ends with either .fits, .tif, or .mat)
vol_params.vol_sz    = [500,500,100];                                      % Volume size to sample (in microns)
vol_params.vol_depth = 250;                                                % Set the depth of imaging (depth at the middle of the simulated volume)
...
psf_params           = getDefaultPSFParams('bessel');                      % Get default bessel imaging parameters
